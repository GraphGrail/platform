$(document).ready(function () {
    ...
    $('div.component-field-repeatable .remove-repeatable').on('click', function () {
        ...
        var last = parent.find('.component-field-repeatable').last();
    ...
    $(".ui-sortable").sortable({
        ...
        start: function start(a, ui) {
            var parent = ui.item.closest('.ui-sortable')[0];
            var another = $('.ui-sortable').not('.' + parent.className.split(' ')[1]);
            another.addClass('prepare-drop');
        },
        stop: function stop(a, ui) {
            $('.prepare-drop').removeClass('prepare-drop');
        },
        receive: function receive(event, ui) {
            var head = ui.item.children('.m-portlet__head').next();
